...
define([
    ...
    function ($, loadHotCourse, sanhai_evaluate, money, pageBar,intoPtCourse,dialogs,popBox,common) {
        ...
        ptCourseDetailsDo.prototype.init = function () {
           ...
           /* var startTime = ptCourseDetailData.startTime;
            ...
            for (var k = 0; k < course.data.arrangeCourseList.length; k++) {
                ...
                html+="<td>";
                html+="<div class='tl'>";
                html+="<a href=\"/site/videoCourse/"+classdata.videoId+"/detail.htm\" target=\"_blank\"><strong >"+classdata.theme+"</strong></a>";
                html+="</div>";
                html+="</td>";
                /*html+="<td>";
                html+="<div class='tc'>";
            ...
            var isExistVideoCard=ptCourseDetailData.isExistVideoCard;
            if(isExistVideoCard==0){
                $('.stu_btn').removeClass('hide');
                $('.buy_btn').addClass('hide');

            }else{
                $('.stu_btn').addClass('hide');
                $('.buy_btn').removeClass('hide');
            }
        };
        ...
        addShoppingCart = function (obj) {
            var videoId = obj.parent('div').children('input').eq(0).val();
            var auditFlag = 0;
            $.ajax({
        ...
        ptCourseDetailsDo.prototype.userListen = function (classId) {
            ...
            if (time < startTime){
                dialogs._wait("课程未开始，请稍后再试", 2);
                return;
            }
            if (courseStatus != "700104"){
                dialogs._wait("老师还未上课，请稍后再试", 2);
                return;
            }
