define(['jquery', 'sanhai-evaluate', 'base'], function ($, eval) {
    //设置打开的菜单
    ...
    eval.loadevaluate1(url, data1, 1);
            ...
            for (var k = 0; k < val.length; k++) {
                ...
                if (evaluateReys.length > 0) {
                    ...
                    for (var j = 0; j < evaluateReys.length; j++) {
                        var evakeyurl = "";
                        if (evaluateReys[j].ppResId == "" || evaluateReys[j].ppResId == 0 || evaluateReys[j].ppResId == 0) {
        ...
        $('.clickReply').click(function () {
            ...
            if ($this.prev('textarea').val()) {
                ...
                if (ppid == "false" || ppid == 0 || ppid == undefined) {
                    ...
                    url = "<%=basePath%>/file/loadImage/" + ppid + ".r?dim=40";
                }
                $('<dl class="clearfix">' +
