...
define(['jquery'],function($){
     ...
     * @param callback      回调函数
     */
    var showPageBar= function (currPage, totalPages, callback,route) {
        var maxShowPage = 7;
        var pageBarHtml = "";
        if (currPage > 1) {
            pageBarHtml += '<a href="#'+route+'/page'+(currPage - 1 )+'" class="jumpBtn prev" page_number="' + (currPage - 1 ) + '">上一页</a>';
        }
        if (currPage <= 3) {
            var i = 1
            for (; (i < maxShowPage && i < Number(totalPages) + 1); i++) {
                if (i == Number(currPage)) {
                    pageBarHtml += '<a href="#'+route+'/page' + i + '"  class="current" page_number=' + i + ' >' + i + '</a>';
                } else {
                    pageBarHtml += '<a href="#'+route+'/page' + i + '" page_number= ' + i + ' >' + i + '</a>';
                }
            }
            if (i < totalPages) {
            ...
            /*var i = currPage - 3*/
            if (currPage > 5) {
                pageBarHtml += '<a href="#'+route+'/page1"   page_number=1 >1</a>';
                pageBarHtml += '<span>……</span>';
            } else {
                if (currPage == 5) {
                    pageBarHtml += '<a href="#'+route+'/page1"   page_number=1 >1</a>';
        ...
        if (Number(currPage) < Number(totalPages)) {
            pageBarHtml += '<a href="#'+route+'/page' + (Number(currPage) + 1 ) + '" class="jumpBtn next" page_number=' + (Number(currPage) + 1 ) + '>下一页</a>';
        }
        if (totalPages > 1) {
        ...
        $('#listSiteSchoolCon').find("#pageBar").html(pageBarHtml);
        //绑定事件（确定按钮）
        $("#toPage").on("click", function () {
