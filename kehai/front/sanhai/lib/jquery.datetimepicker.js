					...
					.find('.xdsoft_prev,.xdsoft_next')
						.on('mousedown.xdsoft',function() {
							...
							(function arguments_callee1(v) {
								...
								var myMonth = operDate.getMonth();
								var myYear = operDate.getFullYear();
								if( $this.hasClass( options.next ) ) {
									_xdsoft_datetime.nextMonth();
								}else if( $this.hasClass( options.prev ) ) {
									if (year - myYear <= 0 && month-myMonth<=0) return false;
				...
				datetimepicker
					.on('xchange.xdsoft',function( event ) {
						...
						if( !options.allowTimes || !$.isArray(options.allowTimes) || !options.allowTimes.length ) {
							...
							var year = _xdsoft_datetime.currentTime.getFullYear();
							var month = _xdsoft_datetime.currentTime.getMonth();
							var day = _xdsoft_datetime.currentTime.getDate();
							var selectDate = new Date(year, month, day);
							var currDate = new Date();
							var _today = new Date(currDate.getFullYear(), currDate.getMonth(), currDate.getDate());
							var hour = currDate.getHours();

							for( var i=0;i<options.allowTimes.length;i++ ) {
								/*
								if (selectDate > _today){
